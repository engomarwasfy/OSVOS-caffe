...
def get_edge_label(layer):
    ...
    if layer.type == 'Data':
        return 'Batch ' + str(layer.data_param.batch_size)
...
def get_layer_label(layer, rankdir):
    ...
    if layer.type in ['Convolution', 'Deconvolution']:
        ...
        return '"%s%s(%s)"' % (layer.name, separator, layer.type)
...
def get_pydot_graph(caffe_net, rankdir, label_edges=True, phase=None):
    ...
    pydot_graph = pydot.Dot(
        caffe_net.name or 'Net', graph_type='digraph', rankdir=rankdir
    )
